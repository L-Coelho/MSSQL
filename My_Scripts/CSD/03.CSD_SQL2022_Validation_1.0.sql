...
Print 'Creating Stored Procedure CSD_SQL2022_Validation 1.0'

IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME = 'CSD_SQL2022_Validation')
	EXEC ('CREATE PROC [dbo].[CSD_SQL2022_Validation] AS SELECT ''stub version, to be replaced''')
GO
...
-- Description:	<Validation of SQL Server 2022 CSD v1.0>

-- =============================================
ALTER PROCEDURE [dbo].[CSD_SQL2022_Validation]
...
@sqlauthentication Values -> 0 -  mixed mode authentication\ 1 Windows authentication mode
*/
...
BEGIN
    ...
    CREATE TABLE #CSDResults (
        ...
        [Agreed to Value]        NVARCHAR (200)  NOT NULL,
    ...
    SELECT '000' AS [Section],
           'CSD SQL 2022 Validation' AS [Section Heading],
	...
	--AO1.C.1.1.0
    INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
    SELECT 'AO1.C.1.1.0' AS [Section],
           'Surface Area Reduction' AS [Section Heading],
           'Surface Area Reduction' AS [System Value/Parameter],
           'SQL Server offers various configuration options, some of them can be controlled by the `sp_configure` stored procedure. This section contains the listing of the corresponding recommendations.' AS [Description],
           'No value to be set' AS [Agreed to Value],
           'N\A' AS [Result];
	--AO1.C.1.1.1
    ...
    IF @resultadhocqueries = @adhocqueries
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.1' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure Ad Hoc Distributed Queries Server Configuration Option is set to 0.' AS [System Value/Parameter],
               'Enabling Ad Hoc Distributed Queries allows users to query data and execute statements on external data sources. This functionality should be disabled.' AS [Description],
    ...
    IF @resultclr = @clr
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.2' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure CLR Enabled Server Configuration Option is set to 0.' AS [System Value/Parameter],
    ...
    IF @resultcross = @cross
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.3' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure Cross DB Ownership Chaining Server Configuration Option is set to 0.' AS [System Value/Parameter],
               'The `cross db ownership chaining` option controls cross-database ownership chaining across all databases at the instance (or server) level.' AS [Description],
    ...
    IF @resultdatabasemail = @databasemail
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.4' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure Database Mail XPs Server Configuration Option is set to 0.' AS [System Value/Parameter],
    ...
    IF @resultole = @ole
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.5' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure Ole Automation Procedures Server Configuration Option is set to 0.' AS [System Value/Parameter],
    ...
    IF @resultremoteaccess = @remoteaccess
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.6' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure Remote Access Server Configuration Option is set to 0.' AS [System Value/Parameter],
    ...
    IF @resultremoteadmin = @remoteadmin
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.7' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure Remote Admin Connections Server Configuration Option is set to 0.' AS [System Value/Parameter],
    ...
    IF @resultscanprocs = @scanprocs
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.8' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure Scan For Startup Procs Server Configuration Option is set to 0.' AS [System Value/Parameter],
 ...
 --AO1.C.1.1.9
    ...
    IF @resulttrustworthy = @trustworthy
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.9' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure Trustworthy Database Property is set to Off.' AS [System Value/Parameter],
    ...
    IF @resultsqlport = @sqlport
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.10' AS [Section],
    ...
    IF @resulthideinstance = @hideinstance
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.11' AS [Section],'Surface Area Reduction' AS [Section Heading],'Ensure Hide Instance option is set to Yes for Production SQL Server instances.' AS [System Value/Parameter],
    ...
    IF @resultsadisable = @sadisable
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.12' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure the sa Login Account is set to Disabled.' AS [System Value/Parameter],
               'The `sa` account is a widely known and often widely used SQL Server account with sysadmin privileges. This is the original login created during installation and always has the `principal_id=1` and `sid=0x01`.' AS [Description],
...
-- AO1.C.1.1.13
    ...
    IF @resultsarenamed = @sarenamed
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.13' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure the sa Login Account has been renamed.' AS [System Value/Parameter],
               'The `sa` account is a widely known and often widely used SQL Server login with sysadmin privileges. The `sa` login is the original login created during installation and always has `principal_id=1` and `sid=0x01`.' AS [Description],
	...
	IF NOT EXISTS (SELECT name,
        ...
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.14' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure AUTO_CLOSE is set to OFF on contained databases.' AS [System Value/Parameter],
	...
	IF @resultsarenamed = @sarenamed
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.15' AS [Section],'Surface Area Reduction' AS [Section Heading],
    ...
    IF @resultclrstrictsecurity = @clrstrictsecurity
        ...
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.16' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
    ...
    ELSE
        ...
        SELECT 'AO1.C.1.1.16' AS [Section],
               ...
               'NOK' AS [Result];
	--AO1.C.1.1.17
	DECLARE @resultsqlauthentication AS INT;
    SET @resultsqlauthentication = (SELECT CONVERT(INT,SERVERPROPERTY('IsIntegratedSecurityOnly')) as [login_mode]);
    IF @resultsqlauthentication = @sqlauthentication
	        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.17' AS [Section],
               'Surface Area Reduction' AS [Section Heading],
               'Ensure Server Authentication Property is set to Windows Authentication Mode.' AS [System Value/Parameter],
               'Uses **Windows Authentication** to validate attempted connections.' AS [Description],
               '' + CONVERT (VARCHAR, @sqlauthentication) + '' AS [Agreed to Value],
               'OK' AS [Result];
    ELSE
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.1.1.17' AS [Section],
	...
	--AO1.C.2.1.0
    INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
    SELECT 'AO1.C.2.1.0' AS [Section],
           'Authentication and Authorization' AS [Section Heading],
           'Authentication and Authorization' AS [System Value/Parameter],
           'This section contains recommendations related to SQL Servers authentication and authorization mechanisms.' AS [Description],
           'No value to be set' AS [Agreed to Value],
           'N\A' AS [Result];
...
--AO1.C.2.1.1
    ...
    IF @resultguest = @guest
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.2.1.1' AS [Section],
               'Authentication and Authorization' AS [Section Heading],
               'Ensure CONNECT permissions on the guest user is Revoked within all SQL Server databases.' AS [System Value/Parameter],
               'Remove the right of the `guest` user to connect to SQL Server databases, except for `master`, `msdb`, `tempdb`, and, on AWS RDS instances, `rdsadmin`.' AS [Description],
...
--AO1.C.2.1.3
    ...
    IF NOT EXISTS (SELECT *
                   ...
                   WHERE  (grantee_principal_id = SUSER_SID(N'public')
                          ...
                          AND NOT (state_desc = 'GRANT'
                                   ...
                                   AND major_id = 5))
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.2.1.4' AS [Section],
               'Authentication and Authorization' AS [Section Heading],
               'Ensure only the default permissions specified by Microsoft are granted to the public server role.' AS [System Value/Parameter],
               ...
               'Ensure only the default permissions specified by Microsoft are granted to the public server role' AS [Agreed to Value],
               'OK' AS [Result];
    ELSE
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.2.1.4' AS [Section],
	...
	--AO1.C.3.1.0
    INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
    SELECT 'AO1.C.3.1.0' AS [Section],
           'Password Policies' AS [Section Heading],
           'Password Policies' AS [System Value/Parameter],
           'This section contains recommendations related to SQL Servers password policies.' AS [Description],
           'No value to be set' AS [Agreed to Value],
           'N\A' AS [Result];
...
--AO1.C.3.1.2
    ...
    IF @resultpolicy = @sqlpolicy
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.3.1.2' AS [Section],
               'Password Policies' AS [Section Heading],
               'Ensure CHECK_POLICY Option is set to ON for All SQL Authenticated Logins.' AS [System Value/Parameter],
               'Applies the same password complexity policy used in Windows to passwords used inside SQL Server.' AS [Description],
	...
	--AO1.C.4.1.0
    INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
    SELECT 'AO1.C.4.1.0' AS [Section],
           'Auditing and Logging' AS [Section Heading],
           'Auditing and Logging' AS [System Value/Parameter],
           'This section contains recommendations related to SQL Servers audit and logging mechanisms.' AS [Description],
           'No value to be set' AS [Agreed to Value],
           'N\A' AS [Result];
    ...
    IF @NLogs = @NumLogs
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.4.1.1' AS [Section],
               'Auditing and Logging' AS [Section Heading],
               'Ensure Maximum number of error log files is set to greater than or equal to 12.' AS [System Value/Parameter],
               'SQL Server error log files must be protected from loss. The log files must be backed up before they are overwritten. Retaining more error logs helps prevent loss from frequent recycling before backups can occur.' AS [Description],
    ...
    IF @resulttrace = @trace
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.4.1.2' AS [Section],
               'Auditing and Logging' AS [Section Heading],
               'Ensure Default Trace Enabled Server Configuration Option is set to 1.' AS [System Value/Parameter],
    ...
    IF (SELECT @result) = @Loginauditing
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.4.1.3' AS [Section],
               'Auditing and Logging' AS [Section Heading],
               'Ensure Login Auditing is set to failed logins.' AS [System Value/Parameter],
               'This setting will record failed authentication attempts for SQL Server logins to the **SQL Server Errorlog**. This is the default setting for SQL Server. ' AS [Description],
    ...
    IF @resultsqlaudit = @sqlaudit
        BEGIN
            INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
            SELECT 'AO1.C.4.1.4' AS [Section],
                   'Auditing and Logging' AS [Section Heading],
                   'Ensure SQL Server Audit is set to capture both failed and successful logins.' AS [System Value/Parameter],
	...
	--AO1.C.5.1.0
    INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
    SELECT 'AO1.C.5.1.0' AS [Section],
           'Application Development' AS [Section Heading],
           'Application Development' AS [System Value/Parameter],
           'This section contains recommendations related to developing applications that interface with SQL Server.' AS [Description],
           'No value to be set' AS [Agreed to Value],
           'N\A' AS [Result];
    ...
    WHILE 1 = 1
        BEGIN
            ...
            SET @SqlStatement2 = N'USE ' + QUOTEname(@DB2) + CHAR(13) + CHAR(10) + N'INSERT INTO #Tmpclrassemblysafe
			                     SELECT db_name() AS Database_Name,name,permission_set,permission_set_desc FROM sys.assemblies WHERE is_user_defined = 1 and name <> ''Microsoft.SqlServer.Types'';';
    ...
    IF @resultclrassemblysafe = 0
        INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
        SELECT 'AO1.C.5.1.1' AS [Section],
               'Application Development' AS [Section Heading],
               'Ensure CLR Assembly Permission Set is set to SAFE_ACCESS for All CLR Assemblies.' AS [System Value/Parameter],
	...
	--AO1.C.6.1.0
    INSERT INTO #CSDResults ([Section], [Section Heading], [System Value/Parameter], [Description], [Agreed to Value], [Result])
    SELECT 'AO1.C.6.1.0' AS [Section],
           'Encryption' AS [Section Heading],
           'Encryption' AS [System Value/Parameter],
           'These recommendations pertain to encryption-related aspects of SQL Server.' AS [Description],
           'No value to be set' AS [Agreed to Value],
           'N\A' AS [Result];
